...
export function ingestDocument(
  ...
  // Create document record with stable ID derived from content hash
  const docId = store.addDocument({
    id: stableId("doc", contentHash, filename),
    filename,
  ...
  // Persist chunks with stable IDs derived from document ID + chunk index
  for (let i = 0; i < chunks.length; i++) {
    store.addChunk({
      id: stableId("chunk", docId, String(i)),
      document_id: docId,
      chunk_index: i,
