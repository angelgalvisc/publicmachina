...
import { appendDailyNote, listSimulationHistory, updateSimulationHistoryRecord } from "./assistant-workspace.js";
import type {
  AssistantTaskState,
...
export interface AssistantToolRuntime {
  ...
  taskState: AssistantTaskState | null;
  mock?: boolean;
...
export async function executeAssistantTool(
  ...
  switch (tool) {
    case "design_simulation":
      return designSimulationTool(args, runtime);
    case "run_simulation":
      return runSimulationTool(args, runtime);
    case "query_simulation":
      return querySimulationTool(args, runtime);
    case "interview_actor":
      return interviewActorTool(args, runtime);
    case "generate_report":
      return generateReportTool(args, runtime);
    case "list_history":
      return listHistoryTool(args, runtime);
    case "export_agent":
      return exportAgentTool(args, runtime);
    case "switch_provider":
      return switchProviderTool(args, runtime);
  }
}
...
async function runSimulationTool(
  ...
  if (runtime.workspace && designed.historyRecordId) {
...
async function generateReportTool(
  ...
  try {
    ...
    const reportPath = join(dirname(target.dbPath), "report.md");
    const lines = [
...
async function listHistoryTool(
  ...
  if (!runtime.workspace) {
    return {
      status: "error",
      summary: "Workspace memory is disabled, so there is no simulation history to list.",
    };
  }
...
async function exportAgentTool(
  ...
  try {
    const actor = resolveActorByName(store, target.runId, actorName);
    const outDir = stringifyArg(args.outDir) ?? join(dirname(target.dbPath), `ckp-export-${actor.handle ?? actor.id}`);
